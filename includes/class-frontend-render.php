...
if ( ! class_exists( 'Cielo_Frontend_Render' ) ) {
	...
	class Cielo_Frontend_Render {
		...
		public function render_fields() {
			...
			// Call the function statically since we updated it below
			$fields_data = self::get_merged_fields( $product->get_id() );
		...
		// Fixed: Made public static so cart handler can read data without re-triggering hooks
		public static function get_merged_fields( $product_id ) {
						...
						if ( is_array( $template_fields ) ) {
							foreach ( $template_fields as $field ) {
								if ( ! in_array( $field['key'], $tracked_keys ) ) {
									$field['origin_name'] = $template->post_title; 
		...
		private function render_single_field( $field ) {
			...
			switch ( $field['type'] ) {
				...
				case 'file':
					...
					echo '<div class="cielo-file-preview" style="margin-top:10px;"></div>'; 
		...
		public function enqueue_assets() {
			if ( ! is_product() ) return;
			// Fixed paths: Enqueued both JS and CSS
			wp_enqueue_script( 'cielo-frontend-fields', plugins_url( '../assets/js/frontend-fields.js', __FILE__ ), array(), '2.0.0', true );
			wp_enqueue_style( 'cielo-frontend-styles', plugins_url( '../assets/css/frontend-fields.css', __FILE__ ), array(), '2.0.0' );
		}
	}
