...
if ( ! class_exists( 'Cielo_Cart_Handler' ) ) {
	...
	class Cielo_Cart_Handler {
		...
		// Fixed: Now reads the actual field data directly from the frontend engine without instantiating it!
		private function get_field_config_from_db( $product_id, $target_key ) {
			if ( class_exists('Cielo_Frontend_Render') ) {
				$fields_data = Cielo_Frontend_Render::get_merged_fields( $product_id );
				if ( ! empty( $fields_data['fields'] ) ) {
			...
			return array('priceModifier' => 0, 'feeType' => 'per_item'); 
		}
	}
